...
import 'package:medcave/Users/Mobilescreens/features/ambulance_tracking/presentation/pages/driver/widget/location_distance.dart';
...
class _AmbulanceDriverScreenActiveState
  ...
  void _calculateDistanceAndTime() {
    if (_patientLatitude != 0.0 && _patientLongitude != 0.0 &&
        _driverLatitude != 0.0 && _driverLongitude != 0.0) {
  ...
  Future<void> _updateDriverLocationInFirestore() async {
    try {
      ...
      await _firestore.collection('ambulanceRequests').doc(widget.rideId).update({
        'driverLocation': {
          'latitude': _driverLatitude,
          'longitude': _driverLongitude,
          'timestamp': FieldValue.serverTimestamp(),
  ...
  Widget build(BuildContext context) {
    ...
    final locationData = _rideData['location'] as Map<String, dynamic>? ?? {};
    final String address = locationData['address'] as String? ?? 'Unknown location';
